# Evaluations (`activeagents-evals`)

`activeagents-evals` is the evaluation core the dashboard's
[scenario evaluations](/framework/dashboard#scenario-evaluations) run on,
released as its own gem so any Rails app — on ActiveAgent, on RubyLLM, or on a
client of its own — can replay a list of tasks across models, score the
answers, and be told which tasks its agent cannot do yet and why.
...
The gem depends on `activesupport` only. It never talks to a model itself: you
hand it one callable that runs your agent, and optionally one that asks a judge
...
`ActiveAgents::Evals::Replay` (or a hash with the same keys). With ActiveAgent
...
ActiveAgents::Evals::Runner.new(scenarios:, models:, replay:, judge: judge, instructions: SupportAgent.instructions).call
```
...
The gem's README in the repository (`evals/README.md`) has the full class
table and the suite format.
