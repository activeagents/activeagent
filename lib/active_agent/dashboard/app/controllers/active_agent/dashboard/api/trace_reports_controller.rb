...
module ActiveAgent
  module Dashboard
    module Api
      ...
      # on ActiveAgent::Dashboard.trace_model), scoped to the caller's tenant.
      ...
      class TraceReportsController < BaseController
        ...
        def show
          trace = traces_scope.find_by(trace_id: params[:id]) ||
            traces_scope.where("trace_id LIKE ?", "#{ActiveAgent::Dashboard.trace_model.sanitize_sql_like(params[:id].to_s)}%").first ||
