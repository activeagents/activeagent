...
# index never treats two NULLs as equal. All three columns are NULL for a run
# the dashboard executed. Emitted for an install whose dashboard tables
# predate published reports; the create-table migration carries the same
# columns for a fresh install. Each step is guarded, so the migration is safe
# to re-run.
class AddEvaluationReportIdentity < ActiveRecord::Migration[7.2]
  ...
  def up
    ...
    %i[external_tenant external_run_id external_report_digest].each do |column|
      add_column table, column, :string unless column_exists?(table, column)
    end
