...
class SchemaToolsTest < ActiveSupport::TestCase
  ...
  test "an enum checks every member of an IN filter" do
    Post.create!(title: "Live one", content: "x", user: @alice, state: "live")

    assert_equal({ count: 3 }, EnumPostTools.call("count_posts", state: [ "draft", "live" ]))

    # `where(state: ["live", "pending"])` quietly drops the undefined name and
    # answers for "live" alone, as though that were what was asked.
    result = EnumPostTools.call("count_posts", state: [ "live", "pending" ])
    refute result.key?(:count), "an IN filter with an undefined value must not return a count"
    assert_match "`pending` is not a valid `state`", result[:error]
  end
