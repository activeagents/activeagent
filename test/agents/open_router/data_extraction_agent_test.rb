...
module OpenRouter
  class DataExtractionAgentTest < ActiveSupport::TestCase
    VCR_FOLDER = "open_router/data_extraction_agent"

    test "throw error when output schema not found" do
      VCR.use_cassette("#{VCR_FOLDER}/schema_not_found") do
        assert_raises(ActiveAgent::Errors::SchemaNotFoundError) do
          OpenRouter::DataExtractionAgent.with(
    ...
    #     prompt = OpenRouter::DataExtractionAgent.describe_cat_image
    ...
    #   VCR.use_cassette("#{VCR_FOLDER}/describe_cat_image_generation_response") do
    ...
    test "parse_resume creates a multimodal prompt with file data" do
      ...
      VCR.use_cassette("#{VCR_FOLDER}/parse_resume_generation_response") do
        response = prompt.generate_now
        doc_example_output(response)
    ...
    test "parse_resume creates a multimodal prompt with file data with structured output schema" do
      prompt = nil
      VCR.use_cassette("#{VCR_FOLDER}/parse_resume_with_structured_output") do
      ...
      VCR.use_cassette("#{VCR_FOLDER}/parse_resume_generation_response_with_structured_output") do
    ...
    #   prompt = nil
    #   VCR.use_cassette("#{VCR_FOLDER}/parse_chart") do
    ...
    #   VCR.use_cassette("#{VCR_FOLDER}/parse_chart_generation_response") do
    #     response = prompt.generate_now
    #     doc_example_output(response)
    ...
    #   prompt = nil
    #   VCR.use_cassette("#{VCR_FOLDER}/parse_chart_with_structured_output") do
    #     sales_chart_path = Rails.root.join("..", "..", "test", "fixtures", "images", "sales_chart.png")
    ...
    #   VCR.use_cassette("#{VCR_FOLDER}/parse_chart_generation_response_with_structured_output") do
