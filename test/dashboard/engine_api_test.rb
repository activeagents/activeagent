...
class DashboardEngineApiTest < ActionDispatch::IntegrationTest
  def setup
    ActiveAgent::Dashboard::Agent.delete_all
    ActiveAgent::TelemetryTrace.delete_all
  end

  def teardown
    ActiveAgent::Dashboard.quota_checker = nil
    ActiveAgent::Dashboard.usage_recorder = nil
    ActiveAgent::Dashboard.execution_enabled = true
  end

  def create_agent(name: "Support", **attributes)
    ActiveAgent::Dashboard::Agent.create!(
  ...
  test "creating an agent versions it" do
    ...
    agent = ActiveAgent::Dashboard::Agent.find(JSON.parse(response.body).dig("agent", "id"))
  ...
  test "the host app's quota checker can refuse an execution" do
    agent = create_agent
    ActiveAgent::Dashboard.quota_checker = ->(_owner, kind) { "Out of runs" if kind == :execution }
  ...
  test "execution can be turned off entirely" do
    agent = create_agent
    ActiveAgent::Dashboard.execution_enabled = false
  ...
  test "a run reports usage back to the host app" do
    agent = create_agent
    recorded = []
    ActiveAgent::Dashboard.usage_recorder = ->(owner, kind) { recorded << [ owner, kind ] }
  ...
  test "metrics and traces read the same store the ingest endpoint writes" do
    ActiveAgent::TelemetryTrace.create_from_payload(sample_trace_payload)
...
class DashboardOwnershipTest < ActiveSupport::TestCase
  def teardown
    ActiveAgent::Dashboard.user_class = nil
    ActiveAgent::Dashboard.account_class = nil
  end

  test "nothing is owned when the host app configures no owner model" do
    assert_nil ActiveAgent::Dashboard::Agent.owner_association
    assert_equal ActiveAgent::Dashboard::Agent.all.to_sql,
      ActiveAgent::Dashboard::Agent.for_owner(nil).to_sql
  end

  test "agents prefer a user and keys prefer an account" do
    ActiveAgent::Dashboard.user_class = "User"
    ActiveAgent::Dashboard.account_class = "User" # the dummy app has no Account

    agent_candidates = ActiveAgent::Dashboard::Agent.owner_candidates
    key_candidates = ActiveAgent::Dashboard::ApiKey.owner_candidates
  ...
  test "the table prefix is configurable so a host app keeps its own names" do
    assert_equal "active_agent_agents", ActiveAgent::Dashboard::Agent.table_name

    ActiveAgent::Dashboard.table_name_prefix = ""
    ActiveAgent::Dashboard::Agent.reset_table_name

    assert_equal "agents", ActiveAgent::Dashboard::Agent.table_name
  ensure
    ActiveAgent::Dashboard.table_name_prefix = "active_agent_"
    ActiveAgent::Dashboard::Agent.reset_table_name
  end
end
